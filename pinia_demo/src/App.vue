<template>
  <div>
    ...
    <div>
      {{ userStore.name }},{{ userStore.age }},{{ userStore.level }},{{
        userStore.home
      }},{{ userStore }}
    </div>
    ...
    <div>
      访问getters的doubleCountAddone:{{ counterStore.doubleCountAddone }}
    </div>
    ...
    <div style="background-color: aqua; margin-top: 20px">actions</div>
    <button @click="doincrement">调用actions的increment让count自增</button>
    <button @click="doincrementNum">调用actions的incrementNum让count每次增加指定个数</button>
    <button @click="dofetchHomeMultidata">调用actions的fetchHomeMultidata异步获取数据</button>
    <button @click="dofetchHomeMultidata2">调用actions的fetchHomeMultidata2异步获取数据</button>
  </div>
</template>
...
console.log(
  `是不是reactive类型：isProxy:${isProxy(counterStore)},isReactive:${isReactive(
    counterStore
  )},isRef:${isRef(counterStore)}`
);
...
function doincrementNum(){
  counterStore.incrementNum(10)
}
function dofetchHomeMultidata(){
  counterStore.fetchHomeMultidata().then(res => {
    console.log("响应回来1："+res)
  })
}
function dofetchHomeMultidata2(){
  counterStore.fetchHomeMultidata2().then(res => {
    console.log("响应回来2："+res)
  })
}
